      ...
      >
            ...
            <ReactMarkdown
              ...
              components={{
                code({ node, inline, className, children, ...props }: any) {
                    ...
                    return (
                      <div className="relative bg-[#1e1e2e] rounded-lg my-4 overflow-hidden shadow-lg">
                        <div className="flex items-center justify-between px-4 py-2 bg-[#2d2d3b] text-xs text-gray-300">
                          <span>{match[1]}</span>
                          <button onClick={() => navigator.clipboard.writeText(codeString)} className="hover:text-white flex items-center gap-1 transition-colors">
                            <Copy size={14} /> Copy
                          </button>
                        </div>
                  ...
                  return <code className="bg-black/10 rounded px-1.5 py-0.5 font-mono text-sm" {...props}>{children}</code>;
                },
              }}
            >
              {message.content}
        ...
        <div className={`absolute ${isUser ? "-left-10" : "-right-16"} top-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1`}>
          ...
          {!isUser && !message.isStreaming && (
            <>
              <button onClick={handleCopyText} className="p-1.5 text-gray-400 hover:text-[#1a7a4c] bg-white rounded-full shadow border border-gray-100" title="Copy">
                {copiedText ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
              </button>
              <div className="bg-white rounded-full shadow border border-gray-100">
